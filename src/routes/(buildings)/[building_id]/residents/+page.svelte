<script lang="ts">
  ...
  export let data: PageData;
	...
	const triggerCreatetModal = () => {
		formData.name = '';
		formData.position = '';
		formData.percentage.house = 0;
		formData.percentage.park = 0;
		modalStore.trigger(newResidentModal);
	}
  ...
  const newResidentModal: ModalSettings = {
	...
	const triggerEditModal = (resident: Resident) => {
...
<div class="flex flex-row justify-between w-full">
	...
	<button class="btn variant-ghost-primary font-bold" on:click={() => triggerCreatetModal()}>
