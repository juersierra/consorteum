<script lang="ts">
	...
	export let csvData: Array<any> = [];
	export let filename: string;
	export let name: 'bills' | 'residents';
</script>

<!--<CSVDownloader type={'button'} data={csvData} filename={filename} bom={true} >-->
<button
	on:click={() => {
		downloadCsv(csvData, csvData, filename);
	}}
	class="btn variant-ghost-secondary font-bold"
>
	<p class="hidden md:block invisible md:visible mr-2">
		{name == 'bills' ? 'GASTOS' : 'RESIDENTES'}
	</p>
