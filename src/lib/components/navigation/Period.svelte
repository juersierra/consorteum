<script lang="ts">
  import { buildingStore } from '$lib/store/building.store';
	import { periodStore } from "$lib/store/period.store";
	import { getDrawerStore, getModalStore, type ModalSettings } from "@skeletonlabs/skeleton";
  const modalStore = getModalStore();
	

  const periodModal: ModalSettings = {
    type: 'component',
    component: 'periodModal',
    response: (r:string) => {
      periodStore.periodHandler.getPeriod($buildingStore.data?.id, r)
    },
  };

	const months = [
		"Enero",
		"Febrero",
		"Marzo",
		"Abril",
		"Mayo",
		"Junio",
		"Julio",
		"Agosto",
		"Septiembre",
		"Octubre",
		"Noviembre",
		"Diciembre"
	]
</script>

<li class="variant-ghost-secondary rounded-sm ring-4 py-1">
	<button on:click={() => modalStore.trigger(periodModal)} class="flex flex-row justify-between w-full">
		<p>{$periodStore.data ? `${months[$periodStore.data.month]}/${$periodStore.data.year}` : '...' }</p>
		<svg viewBox="0 0 24 24" class="h-6">
      <g fill="none">
        <path d="M24 0v24H0V0zM12.593 23.258l-.011.002l-.071.035l-.02.004l-.014-.004l-.071-.035c-.01-.004-.019-.001-.024.005l-.004.01l-.017.428l.005.02l.01.013l.104.074l.015.004l.012-.004l.104-.074l.012-.016l.004-.017l-.017-.427c-.002-.01-.009-.017-.017-.018m.265-.113l-.013.002l-.185.093l-.01.01l-.003.011l.018.43l.005.012l.008.007l.201.093c.012.004.023 0 .029-.008l.004-.014l-.034-.614c-.003-.012-.01-.02-.02-.022m-.715.002a.023.023 0 0 0-.027.006l-.006.014l-.034.614c0 .012.007.02.017.024l.015-.002l.201-.093l.01-.008l.004-.011l.017-.43l-.003-.012l-.01-.01z" />
        <path fill="currentColor" d="M16 3a1 1 0 0 1 .993.883L17 4v1h2a2 2 0 0 1 1.995 1.85L21 7v12a2 2 0 0 1-1.85 1.995L19 21H5a2 2 0 0 1-1.995-1.85L3 19V7a2 2 0 0 1 1.85-1.995L5 5h2V4a1 1 0 0 1 1.993-.117L9 4v1h6V4a1 1 0 0 1 1-1m3 9H5v7h14zm0-5H5v3h14z" />
      </g>
    </svg>
	</button>
</li>